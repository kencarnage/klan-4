...
const clearActions = () => {
	getCurrentTab().then((response) => {
		...
		actions = [
			...
			{title:"Help", desc:"Get help with klan on GitHub", type:"action", action:"url", url:"https://github.com/", emoji:true, emojiChar:"🤔", keycheck:false},
...
chrome.action.onClicked.addListener((tab) => {
	chrome.tabs.sendMessage(tab.id, {request: "open-klan"});
});

// Listen for the open klan shortcut
chrome.commands.onCommand.addListener((command) => {
	if (command === "open-klan") {
		getCurrentTab().then((response) => {
			if (!response.url.includes("chrome://") && !response.url.includes("chrome.google.com")) {
				chrome.tabs.sendMessage(response.id, {request: "open-klan"});
			} else {
				chrome.tabs.create({
					url: "./newtab.html"	
				}).then(() => {
...
// Restore the new tab page (workaround to show klan in new tab page)
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => resetklan());
chrome.tabs.onCreated.addListener((tab) => resetklan());
chrome.tabs.onRemoved.addListener((tabId, changeInfo) => resetklan());
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	switch (message.request) {
		...
		case "close-klan":
			getCurrentTab().then((response) => {
				chrome.tabs.sendMessage(response.id, {request: "close-klan"});
			});
			break;
		}
});
